...
static int cuda_tensor_transfer_ensure(void) {
    cudaError_t err;
    if (!g_tensor_transfer_stream) {
        err = cudaStreamCreateWithFlags(&g_tensor_transfer_stream, cudaStreamNonBlocking);
        if (err != cudaSuccess) {
            fprintf(stderr, "ds4: CUDA tensor transfer stream creation failed: %s\n",
                    cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
    ...
    if (!g_tensor_transfer_done_event) {
        err = cudaEventCreateWithFlags(&g_tensor_transfer_done_event, cudaEventDisableTiming);
        if (err != cudaSuccess) {
            fprintf(stderr, "ds4: CUDA tensor transfer done event creation failed: %s\n",
                    cudaGetErrorString(err));
            (void)cudaGetLastError();
            return 0;
        }
...
extern "C" int ds4_gpu_tensor_write_async(ds4_gpu_tensor *tensor, uint64_t offset, const void *data, uint64_t bytes) {
    ...
    cudaError_t err = cudaMemcpyAsync((char *)tensor->ptr + offset,
                                      ...
                                      0);
    return cuda_ok(err, "tensor async write");
}

extern "C" int ds4_gpu_tensor_read_async(const ds4_gpu_tensor *tensor, uint64_t offset, void *data, uint64_t bytes) {
    if (!tensor || (!data && bytes != 0) || offset > tensor->bytes || bytes > tensor->bytes - offset) return 0;
    if (bytes == 0) return 1;
    if (!g_tensor_transfer_active && !ds4_gpu_begin_transfer_from_compute()) return 0;
    cudaError_t err = cudaMemcpyAsync(data,
                                      (const char *)tensor->ptr + offset,
                                      (size_t)bytes,
                                      cudaMemcpyDeviceToHost,
                                      g_tensor_transfer_stream);
    if (!cuda_ok(err, "tensor async read")) return 0;
    err = cudaEventRecord(g_tensor_transfer_done_event, g_tensor_transfer_stream);
    if (!cuda_ok(err, "tensor async read event")) return 0;
    g_tensor_transfer_done_recorded = 1;
    return 1;
}
...
__global__ static void moe_gate_up_mid_decode_ptrs_qwarp32_kernel(
        float *gate_out,
        float *up_out,
        float *mid_out,
    ...
    for (uint32_t rr = 0; rr < 4u; rr++) {
        ...
        float up = 0.0f;
        for (uint32_t b = lane; b < xq_blocks; b += 8u) {
            gate += dev_dot_iq2_xxs_q8_K_block_lut(gr + b, xq + b, s_iq2_grid, s_iq2_signs);
            up += dev_dot_iq2_xxs_q8_K_block_lut(ur + b, xq + b, s_iq2_grid, s_iq2_signs);
        }
        gate = quarter_warp_sum_f32(gate, lane);
        up = quarter_warp_sum_f32(up, lane);
        if (lane == 0) {
...
__global__ static void moe_down_ptrs_qwarp32_kernel(
        float *down_out,
    ...
    uint32_t slot = blockIdx.y;
    if (slot >= n_expert) return;
    const char *down_base = down_ptrs[slot];
    const cuda_block_q8_K *xqb = midq + (uint64_t)slot * midq_blocks;
    for (uint32_t rr = 0; rr < 4u; rr++) {
        uint32_t row = blockIdx.x * 128u + row_lane + rr * 32u;
        if (row >= out_dim) continue;
...
extern "C" int ds4_gpu_routed_moe_one_cached_experts_tensor(
        ds4_gpu_tensor *out,
        ds4_gpu_tensor *gate,
        ds4_gpu_tensor *up,
        ds4_gpu_tensor *mid,
        ...
        float clamp,
        const ds4_gpu_tensor *x) {
    if (!out || !gate || !up || !mid || !down || !model_map || !selected_host || !weights || !x ||
    ...
    for (uint32_t i = 0; i < n_expert; i++) {
        ...
        host_ptrs[i] = gate_w;
        host_ptrs[6u + i] = up_w;
        host_ptrs[12u + i] = down_w;
    }

    const uint64_t ptr_bytes = 18ull * sizeof(host_ptrs[0]);
    const char **dev_ptrs = (const char **)cuda_tmp_alloc(ptr_bytes, "cached routed expert pointer table");
    if (!dev_ptrs) return 0;
    if (!cuda_ok(cudaMemcpy(dev_ptrs, host_ptrs, (size_t)ptr_bytes, cudaMemcpyHostToDevice),
                 "cached routed expert pointer upload")) {
        return 0;
    }
    ...
    int ok = cuda_ok(cudaGetLastError(), "cached routed_moe x quantize launch");
    if (ok) {
        ...
        moe_gate_up_mid_decode_ptrs_qwarp32_kernel<<<qgrid, 256>>>(
            ...
            n_expert,
            clamp);
        ok = cuda_ok(cudaGetLastError(), "cached routed_moe gate/up launch");
    }
    if (ok) {
        dim3 midq_grid(midq_blocks, n_expert, 1);
        q8_K_quantize_kernel<<<midq_grid, 256>>>(midq, (const float *)mid->ptr, expert_mid_dim, n_expert);
        ok = cuda_ok(cudaGetLastError(), "cached routed_moe mid quantize launch");
    }
    if (ok) {
        ...
        moe_down_ptrs_qwarp32_kernel<<<dgrid, 256>>>(
            ...
            out_dim,
            n_expert);
        ok = cuda_ok(cudaGetLastError(), "cached routed_moe down launch");
    }
    if (ok) {
...
extern "C" int ds4_gpu_routed_moe_batch_q8_tensor(ds4_gpu_tensor *out, ds4_gpu_tensor *gate, ds4_gpu_tensor *up, ds4_gpu_tensor *mid, ds4_gpu_tensor *down, const void *model_map, uint64_t model_size, uint64_t gate_offset, uint64_t up_offset, uint64_t down_offset, uint32_t gate_type, uint32_t down_type, uint64_t gate_expert_bytes, uint64_t gate_row_bytes, uint64_t down_expert_bytes, uint64_t down_row_bytes, uint32_t expert_in_dim, uint32_t expert_mid_dim, uint32_t out_dim, const ds4_gpu_tensor *selected, const ds4_gpu_tensor *weights, uint32_t n_expert, float clamp, const ds4_gpu_tensor *xq, uint32_t n_tokens, bool *mid_is_f16) {
    (void)out; (void)gate; (void)up; (void)mid; (void)down;
    (void)model_map; (void)model_size; (void)gate_offset; (void)up_offset; (void)down_offset;
    (void)gate_type; (void)down_type; (void)gate_expert_bytes; (void)gate_row_bytes;
    (void)down_expert_bytes; (void)down_row_bytes; (void)expert_in_dim; (void)expert_mid_dim;
    (void)out_dim; (void)selected; (void)weights; (void)n_expert; (void)clamp;
    (void)xq; (void)n_tokens;
    if (mid_is_f16) *mid_is_f16 = false;
    return 0;
}
extern "C" int ds4_gpu_routed_moe_batch_q8_resident_bf16_tensor(ds4_gpu_tensor *out_bf16, ds4_gpu_tensor *gate, ds4_gpu_tensor *up, ds4_gpu_tensor *mid, ds4_gpu_tensor *midq, ds4_gpu_tensor *experts, const ds4_gpu_tensor *gate_weights, const ds4_gpu_tensor *up_weights, const ds4_gpu_tensor *down_weights, uint32_t gate_type, uint32_t down_type, uint64_t gate_expert_bytes, uint64_t gate_row_bytes, uint64_t down_expert_bytes, uint64_t down_row_bytes, uint32_t expert_in_dim, uint32_t expert_mid_dim, uint32_t out_dim, const ds4_gpu_tensor *selected, const ds4_gpu_tensor *weights, uint32_t n_expert, float clamp, const ds4_gpu_tensor *xq, uint32_t n_tokens, uint32_t storage_experts) {
    (void)out_bf16; (void)gate; (void)up; (void)mid; (void)midq; (void)experts;
    (void)gate_weights; (void)up_weights; (void)down_weights;
    (void)gate_type; (void)down_type; (void)gate_expert_bytes; (void)gate_row_bytes;
    (void)down_expert_bytes; (void)down_row_bytes; (void)expert_in_dim; (void)expert_mid_dim;
    (void)out_dim; (void)selected; (void)weights; (void)n_expert; (void)clamp;
    (void)xq; (void)n_tokens; (void)storage_experts;
    return 0;
}
